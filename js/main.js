        ...
        if (playerChoice === computerChoice) {
            ...
            for (const i of combos) {
                if (playerChoice === i[0] && computerChoice === i[1]) {
                    ++player;
                    roundResult.textContent = 'You win! ${capitalizeWord(i[0])} beats ${capitalizeWord(i[1])}';
                    updateScore("p");
                } else if (computerChoice == i[0] && playerChoice === i[1]) {
                    ++computer;
                    roundResult.textContent = 'You lose! ${capitalizeWord(i[0])} beats ${capitalizeWord(i[1])}}';
                    updateScore("c") ;
                }
            }
        }
...
function capitalizeWord(string) {
    //expects a string and return a string with the first letter capitalized
